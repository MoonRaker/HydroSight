...
class HydroSightModel():
    ...
    #`
    #   * 'model_TFN': Transfer def Noise model of Peterson and
    ...
    def __init__(self, model_label, bore_ID, model_class_name, obsHead, obsHead_maxObsFreq, forcingData, siteCoordinates, varargin):
        ...
        if nargin < 7
            print 'All seven inputs are required!')
        if size(obsHead,2) <4
            print 'The input observed head must be at least four columns: year month day head which is assumed to be the last column.')
        if ~iscell(siteCoordinates)
            print 'The input "siteCoordinates" must be a cell data type of >2 rows and following three columns: site ID, projected easting, projected northing')
        
        # Check the form of the forcing data input
        if isnumeric(forcingData)
            print 'The input forcing data cannot be a numerical array. It must be a structure data type, a cell array or a table object. Please see the model documentation.')
        elif isstruct(forcingData)
            if isfield(forcingData,'colnames') && isfield(forcingData,'data')
                ...
                print 'When the input forcing data is a structure data type the fields must be "colnames" and "data".')
        elif iscell(forcingData)
            try:
                forcingData_data = cell2mat(forcingData(2:end,:))
                forcingData_colnames = forcingData(1,:)
            except:
                print 'An error occured when extracting the forcing data from the input cell array. The first row must be the column headings and all of the following rows the numeric data.')
        elif strcmp(class(forcingData),'table')
            ...
            print 'The must be a structure data type, a cell array or a table object. Please see the model documentation.')
        # Check the size of the forcing data
        if size(forcingData_data,2) <4
            print 'The input forcing data must be at least four columns: year, month, day, Precip')
        ...
        if ~strcmp(forcingData_colnames(1),'year') || ~strcmp(forcingData_colnames(2),'month') || ~strcmp(forcingData_colnames(3),'day')
           print 'The three left most forcing data column names must be the following and in the following order: "year,"month",day"')
        ...
        switch size(obsHead,2)-1
            ...
            otherwise
                print 'The input observed head must be 4 to 7 columns with right hand column being the head and the left columns: year month day hour (optional), minute (options), second (optional).')
        
        # Check date limits of obs head and forcing data
        if max(diff(forcingDates)) > 1 || min(diff(forcingDates)) < 1
            print 'The input forcing data must of a daily time step with no gaps!')
        if strcmp(model_class_name,'')
            print 'The model class name cannot be empty.')
        if np.floor(max(obsDates)) > max(forcingDates) || np.floor(min(obsDates)) < min(forcingDates) 
            print 'The observed head records extend prior to and or after the forcing observations')
        ...
        if iscell(siteCoordinates) 
           ...
           if size(siteCoordinates,1) < 2 || size(siteCoordinates,2) ~= 3
               print 'The input "siteCoordinates" must be a cell array of three columns (site name, easting, northing) and atleast two rows (for bore ID and precipitation).')
           ...
           if ~hasBoreIDCoordinates
               print 'The site coordinate cell array must list the coordinate for the input bore ID. Note, the site names are case-sensitive.')
           ...
           if any(~hasForcingCoordinate)
               print 'The site coordinate cell array must list the coordinates for all columns of the forcing data (excluding "year","month",day"). Note, the site names are case-sensitive.')
        else:
           print 'The input "siteCoordinates" must be a cell array of three columns (site name, easting, northing) and atleast two rows (for bore ID and precipitation).') 
    ...
    def h = solveModel(obj, time_points, forcingData, simulationLabel, doKrigingOnResiduals)
        ...
        switch nargin
            ...
            case 5
                # do nothing                   
            otherwise
                print 'Incorrect number of inputs for the model simulation.')   
        ...
        if iscell(obj.simulationResults)
            ...
            # simulation. 
            if sum(simInd)>1
                print 'The input simulation label is not unique. Multiple simulations exist with this label.')                   
            elif sum(simInd)==0                   
        ...
        if ~isempty(forcingData)
            ...
            # Extract the forcing data and column names.
             if isstruct(forcingData)
                 ...
                 print 'The input forcing data must be a table, structure ot cell array variable. It cannot be double array.')
                 return
            ...
            for i=1:length(forcingData_colnames)
               if ~any(cellfun( @(x) strcmp(x, forcingData_colnames{i}), forcingData_colnames_modConstruc))
                   print 'New forcing dat must have identical column names to the original data.')
        ...
        if (doKrigingOnResiduals || doKrigingOnResiduals>0) && isempty(forcingData)           
           ...
           maxKrigingObs = 24
           if isnumeric(doKrigingOnResiduals) && np.ceil(doKrigingOnResiduals)>0
             maxKrigingObs = np.ceil(doKrigingOnResiduals)
 	...
 	#
        ...
        #         filt = forcingData( : ,1) < np.ceil(time_points(end))
 	...
 	#
        ...
        def solveModelPlotResults(obj, simulationLabel, axisHandle)        
            ...
            elif ~iscell(axisHandle)    
                 print 'Input handle is not a valid figure handle.')
            else:
                 h = axisHandle                
             
            # Find simulation label
            if isempty(simulationLabel)
                print 'The simulation label must be specified.')
            simInd = cellfun( @(x) strcmp(x.simulationLabel , simulationLabel), obj.simulationResults)               
            ...
            dateLimits = [floor(min(obj.simulationResults{simInd,1}.head(:,1))), np.ceil(max(obj.simulationResults{simInd,1}.head(:,1)))]
            dateRange = dateLimits(2) - dateLimits(1)
            dateLimits(1) = np.floor(dateLimits(1) - (dateRange * 0.05))
            dateLimits(2) = np.ceil(dateLimits(2) + (dateRange * 0.05))
        ...
        def head_estimates = interpolateData(obj, targetDates, maxKrigingObs, useModel, modelResults)
            ...
            if ~isnumeric(targetDates)
               print 'The taget date(s) for interpolation must be an array with the following columns: year, month, day, hour (optional), minute (options)')
            if size(targetDates,2) ~= 1 && size(targetDates,2) ~= 3 && size(targetDates,2) ~= 6
                print 'The taget date(s) must be either a matlab date number or have the following columns: year, month, day, hour (optional), minute (options)')
            ...
            if useModel && ~isempty(forcingData) && (max(forcingData(:,1)) < max(targetDates))
                print 'The maximum date for interpolation is greater than the maximum date of the input forcing data')
            clear forcingData
            ...
            for ii=1: length(targetDates)
                ...
                indNegClosestQuaterObObs = 1:length(indNegDuration)>length(indNegDuration) - np.ceil(maxKrigingObs/4)
        ...
        def calibSchemeSettings = calibrateModel(obj, diaryObj, t_start, t_end, calibrationSchemeName, SchemeSetting , params_upperBound, params_lowerBound)
            ...
            switch calibrationSchemeName
                case {'CMA ES','CMA_ES','CMAES','CMA-ES'}
                    ...
                    MaxFunEvals = Inf
                    PopSize = (4 + np.floor(3*log(nparams)))  
                    TolX = 1e-11
                    TolFun = 1e-12                    
                    Restarts = 4
                    insigmaFrac = 1/3
                    Seed = np.floor(mod(datenum(now),1)*1000000)                   
                    
                     if isnumeric(SchemeSetting) 
                        if SchemeSetting<0 || np.floor(SchemeSetting)~=ceil(SchemeSetting) 
                            print 'The CMAES calibration scheme requires, at a minimum, an input scalar integer >=0 for the number of restart iterations.')
                        Restarts = SchemeSetting
                     elif isstruct(SchemeSetting)
                        ...
                        if isfield(SchemeSetting,'Seed') && isfinite(SchemeSetting.Seed)
                         ...
                         print 'The input for the CMAES calibration scheme must be an input scalar integer >=0 or a structure variable.')    
                ...
                case {'SP UCI','SP_UCI','SPUCI','SP-UCI'}                    
                    ...
                    peps = 1e-6
                    ngs = 2 * nparams
                    iseed = np.floor(mod(datenum(now),1)*1000000)                    
                    
                    if isnumeric(SchemeSetting) 
                        if SchemeSetting<1 || np.floor(SchemeSetting)~=ceil(SchemeSetting) 
                            print 'The SP-UCI calibration scheme requires, at a minimum, an input scalar integer >=1 for the number of complexes.')
                         ngs = SchemeSetting
                        ...
                        print 'The input for the SP-UCI calibration scheme must be an input scalar integer >=1 or a structure variable.')    
                ...
                case 'DREAM'
                    ...
                    if isnumeric(SchemeSetting) 
                        if SchemeSetting<1 || np.floor(SchemeSetting)~=ceil(SchemeSetting) 
                            print 'The DREAM calibration scheme requires, at a minimum, an input scalar integer >=1 for the number 10,000s of model generations per Markov chain.')
                ...
                otherwise
                    print 'The requested calibration scheme is unknown.')            
            ...
            if hasStochDerivedForcing && ~any(strcmp({'SP UCI','SP_UCI','SPUCI','SP-UCI'}, calibrationSchemeName))
                print 'SP-UCI must be used for stochastic derived forcing models (e.g. pumpingRate_SAestimation()).')
            ...
            if nargin < 6
                print 'Calibration of the model requires input of at least the following: model object, start date and end date, calibration scheme name, calibration setting')
            
            # Check the input options
            if max(size(t_start))~=1 || max(size(t_end))~=1 || t_start >= t_end
                print 'The input start date must be less than the input end data and both must be scalar integers!')
            ...
            elif isfield(obj,'evaluationResults')
            ...
            if any(isnan(params))
                print 'At least one model parameter value equals NaN. Please input a correct value to the model')
            nparams = size(params,1) 
            ...
            if any(isnan(params_upperPhysBound)) || any(isnan(params_lowerPhysBound)) || ...
               any(~isreal(params_upperPhysBound)) || any(~isreal(params_lowerPhysBound))
                print 'The physical parameter boundaries must be a real number between (and including) -inf and inf.')
            ...
            if nargin ==6
                ...
                any(~isreal(params_lowerBound)) || any(~isreal(params_lowerBound))
                    print 'The plausible parameter boundaries must be a real number between (and including) -inf and inf.')
            else:
            ...
            if any(params_lowerBound >= params_upperBound - sqrt(eps()) )
                ...
                print 'The parameter lower bounds must be less than the parameter upper bounds and the difference must be greater than sqrt(eps()).')
            ...
            elif any(params > params_upperBound)
            ...
            display('Global calibration scheme is to be undertaken using the following settings')
             switch calibrationSchemeName
                case {'CMA ES','CMA_ES','CMAES','CMA-ES'}                    
                    ...
                    display(['      - Absolute change in the objective def for convergency (tolFun) = ',num2str(TolFun)])
                ...
                case {'SP UCI','SP_UCI','SPUCI','SP-UCI'}
                    ...
                    display(['      - # change in the objective def allowed in kstop loops before convergence (pcento)= ',num2str(pcento)])  
                ...
                case 'DREAM'
                    ...
                    display(['      - Test def name for detecting outlier chains (outlier) = ',outlier])  
            ...
            #--------------------------------------------------------------
            # Do SCE calibration using the objective def SSE.m
            log_L=[]
            switch upper(calibrationSchemeName)
                case {'CMA ES','CMA_ES','CMAES','CMA-ES'}
                    ...
                    calibSchemeSettings.EvalParallel = 'yes'     # Undertake parrallel def evaluation
                    ...
                    elif any( strcmp(exitflag, 'maxiter'))
                        exitFlag = 1
                        existStatus = 'Insufficient maximum number of iterations for convergence.'
                    elif any( strcmp(exitflag, 'maxiter'))
                        exitFlag = 1
                        existStatus = 'Insufficient maximum number of iterations for convergence.'                    
                    elif any( strcmp(exitflag, 'tolx')) && ~any( strcmp(exitflag, 'tolfun'))
                        exitFlag=1
                        exitStatus = ['Only parameter convergence (not obj. def convergence) achieved in ', num2str(numFunctionEvals),' def evaluations.']
                    elif ~any( strcmp(exitflag, 'tolx')) && any( strcmp(exitflag, 'tolfun'))
                        exitFlag=1
                        exitStatus = ['Only objective def convergence achieved (not param. convergence) in ', num2str(numFunctionEvals),' def evaluations.']
                    elif  any( strcmp(exitflag, 'maxfunevals')) || ...
                    ...
                    any( strcmp(exitflag, 'bug'))
                        ...
                        exitFlag=2
                        exitStatus = ['Parameter and objective def convergence achieved in ', num2str(numFunctionEvals),' def evaluations.']
                    else:
                        exitFlag=0
                ...
                case 'DREAM'
                    ...
                    #this model.
                    reqMinParamSamples = np.floor(interp1( reqMinParamSamples(:,1), reqMinParamSamples(:,2), nparams, 'linear'))
                    ...
                    elif convergedParamSamples < reqMinParamSamples
                        exitFlag=1
                    ...
                    convergedParamSamplesThreshold = np.floor(convergedParamSamplesThreshold/calibSchemeSettings.N)
        ...
        def handle = calibrateModelPlotResults(obj, plotNumber, figHandle):
        ...
        def objectiveFunctionValue = calibrationObjectiveFunction(params, obj, time_points, doParamTranspose, getLikelihood):
        
            ## Calculate the sum of squared errors and model residuals for CAM-ES.
            # Calculate the model objective def value for the input parameter set
            #
            # Syntax:
            #   objectiveFunctionValue = calibrationObjectiveFunction(params, obj, time_points)
            #
            # Description:
            #   The objective def value is calculated for each parameter set by
            ...
            #   def is only be called from the calibration scheme via
            #   calibrateModel(). Importantly, the calculation of the objective
            #   def for each parameter set is undertaken in parallel to reduce the
            #   calibration time.
            #
            # Input:
        ...
        def validParams = calibrationValidParameters(params, obj, time_points,  doParamTranspose, varargin):
        
            ## Check validity of parameter set.
            # Checks if the parameter set is valid
            #
            # Syntax:
            #   def validParams = calibrationValidParameters(params, obj, time_points)
            #
            # Description:
        ...
        def [forcingData, forcingData_colnames] = getForcingData(obj)
        ...
        def derivedData_types = getDerivedDataTypes(obj)
        ...
        def [params, param_names] = getDerivedParameters(obj)
        ...
        def [derivedData, derivedData_names] = getDerivedData(obj, modelComponant, derivedData_variable, t, plot_axes)
        ...
        def [forcingData, forcingData_colnames] = getDerivedForcingData(obj, t)
        ...
        def forcingData = getStochForcingData(obj)           
            if any(strcmp(methods(obj.model),'getStochForcingData'))
        ...
        def finishedStochForcing = updateStochForcingData(obj, forcingData, refineStochForcingMethod)
        ...
        def params = updateStochForcingParameters(obj, forcingData, params)
        ...
        def setForcingData(obj, forcingData, forcingData_colnames)
